...
const Player = () => {
    ...
    const streamStartedAtRef = React.useRef(null);
    ...
    React.useEffect(() => {
        ...
        streamStartedAtRef.current = video.state.stream !== null ? Date.now() : null;
        noSeedersErrorShownRef.current = false;
    }, [video.state.stream]);
    ...
    React.useEffect(() => {
        torrentProbeRef.current = {
            ...
            streamProgress: statistics.streamProgress,
    ...
    React.useEffect(() => {
        const checkDeadTorrent = () => {
            ...
            if (typeof streamStartedAtRef.current !== 'number') {
                streamStartedAtRef.current = Date.now();
                return;
            }

            if ((Date.now() - streamStartedAtRef.current) < TORRENT_DEAD_TIMEOUT_MS) {
                return;
            }

            if (probe.peers <= 0 && probe.speed <= 0.01 && probe.streamProgress < TORRENT_DEAD_PROGRESS_THRESHOLD) {
